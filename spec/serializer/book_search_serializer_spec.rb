...
RSpec.describe 'BookSearchSerializer' do
  it 'can serialize data returned from the book service and weather service', :vcr do
    ...
    expect(book_search_data[:data][:type]).to be_a(String)
    ...
    expect(book_search_data[:data][:attributes][:forecast][:temperature]).to be_a(Numeric)
    expect(book_search_data[:data][:attributes]).to have_key(:total_books_found)
    expect(book_search_data[:data][:attributes][:total_books_found]).to be_a(Integer)
    expect(book_search_data[:data][:attributes]).to have_key(:books)
    expect(book_search_data[:data][:attributes][:books]).to be_a(Array)
    expect(book_search_data[:data][:attributes][:books][0]).to have_key(:isbn)
    expect(book_search_data[:data][:attributes][:books][0][:isbn]).to be_a(Array)
    expect(book_search_data[:data][:attributes][:books][0]).to have_key(:title)
    expect(book_search_data[:data][:attributes][:books][0][:title]).to be_a(String)
    expect(book_search_data[:data][:attributes][:books][0]).to have_key(:publisher)
    expect(book_search_data[:data][:attributes][:books][0][:publisher]).to be_a(Array)
